...
pub fn rotate_overlay(bot: &mut ImgBuf, top: &ImgBuf, x_shift: i32, y_shift: i32, angle: f32) {
    ...
    let min_x = cmp::max(*min(&vec![tl.x as i32, tr.x as i32, bl.x as i32, br.x as i32]), 0) as i32;
    let max_x = cmp::min(*max(&vec![tl.x as i32, tr.x as i32, bl.x as i32, br.x as i32]), bot.width() as i32 - 1) as i32;
    let min_y = cmp::max(*min(&vec![tl.y as i32, tr.y as i32, bl.y as i32, br.y as i32]), 0) as i32;
    ...
    for bot_x in min_x..max_x{
        for bot_y in min_y..max_y {
            ...
            if (top_x >= 0) & (top_y >= 0) & ((top_x as u32) < top.width()) & ((top_y as u32) < top.height()){
                bot.put_pixel(bot_x as u32, bot_y as u32, *top.get_pixel(top_x as u32, top_y as u32));
            }
        }
    }
    let duration = Instant::now().duration_since(start);
    println!("time elapsed:{:?},{:?}",duration.subsec_nanos(),duration.subsec_millis());
}