...
let init () =
  ...
  let state =
    State.create world
      ...
      flag_id flag_protector_id offense_shield_id
  in
  state.world <- world;
  let flag_img =
...
let handle_events (state : State.t) =
  if Sdl.poll_event (Some state.event) then
...
let step state () =
  ...
  Map.iteri state.world.bodies ~f:(fun ~key:id ~data:robot ->
      Option.iter (Map.find state.images id) ~f:(fun image ->
          if not (Set.mem state.invisible id) then
            let w = robot.shape.bounding_box.width in
            let h = robot.shape.bounding_box.height in
            let alpha =
              if robot.collision_group = Ctf_consts.Bots.Offense.coll_group then
                255 / Ctf_consts.Bots.Offense.start_lives
                * state.offense_bot.bot.lives
              else 255
            in
            Display.draw_image_wh state.display ~w ~h ~alpha image
              ~center:robot.pos ~angle:robot.angle));
...
let set_motors (state : State.t) ((bot_name : Bot_name.t), (l_input, r_input)) =
  let make_valid input =
    if Float.O.(Float.abs input < max_input) then input
...
let load_laser state ((bot_name : Bot_name.t), ()) =
  ...
  | Defense ->
      ...
      if
        (not is_loaded)
        && usable state state.defense_bot.bot.last_fire_ts
...
let shoot_laser state ((bot_name : Bot_name.t), ()) =
  ...
  then (
    if Option.is_none state.defense_bot.bot.loaded_laser then
...
let opp_of (state : State.t) (bot_name : Bot_name.t) =
  body_of state (match bot_name with Offense -> Defense | Defense -> Offense)
...
let laser_cooldown_left state ((_bot_name : Bot_name.t), ()) =
  ts_to_ticks
    (Float.max 0.
       (state.defense_bot.bot.last_fire_ts +. Ctf_consts.Laser.cooldown
      -. state.ts))
...
let boost_cooldown_left state ((_bot_name : Bot_name.t), ()) =
  ts_to_ticks
    (Float.max 0.
      ...
      +. Ctf_consts.Bots.Offense.boost_cooldown -. state.ts))
...
let boost state ((bot_name : Bot_name.t), ()) =
  ...
  | Offense ->
      if
        usable state state.offense_bot.bot.last_boost
...
let enhance_border state =
  ...
  state.world <-
    Border.set_border_black_list state.world
...
let offense_has_flag state (_, _) = state.offense_bot.bot.has_flag
