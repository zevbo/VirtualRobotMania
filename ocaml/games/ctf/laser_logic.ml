...
let laser ~(bot : Body.t) =
  ...
  let v = Vec.origin in
  let m = 0.01 in
  Body.create ~pos ~m ~v ~angle ~max_omega:0.
    ~collision_group:Ctf_consts.Laser.coll_group
    ~black_list:Ctf_consts.Laser.black_list Ctf_consts.Laser.shape
...
let update_moving (state : State.t) id =
  ...
  if out_of_frame laser then state.world <- World.remove_body state.world id
  else
    let offense_bodies =
      ...
      Option.is_some
        (List.find offense_bodies ~f:(fun (_id, body) ->
             not
               ...
               = 0)))
    in
    if hit_offense_body then
      let world = World.remove_body state.world id in
      let offense_bot =
        Offense_bot.remove_live ~num_lives:(Map.find_exn state.lasers id).power
...
let update_loaded (state : State.t) id =
  ...
  if t.power > List.length Ctf_consts.Laser.colors then restock_laser state id
  else
...
let update_one (state : State.t) id =
  (if (Map.find_exn state.lasers id).loaded then update_loaded
  else update_moving)
    state id
