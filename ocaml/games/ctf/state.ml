...
type 'a with_id = { bot : 'a; id : World.Id.t }
...
let create world images display offense_bot defense_bot flag_id
    flag_protector_id offense_shield_id =
  {
    world;
    last_step_end = None;
    images;
...
let load_bot_image t id imagefile =
  ...
  let%bind () =
    Process.run_expect_no_output_exn ~prog:"/usr/local/bin/convert"
