...
let main () = 
  ...
  let robot = Body.create shape 1. 1. 5. in  let display = Display.init ~w:500 ~h:500 ~title:"test" in
  let _world = ref (World.of_bodies [robot;]) in
  let image = Display.Image.of_bmp_file display "../../../images/test-robot.bmp" in
  while true do 
