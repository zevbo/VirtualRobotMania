...
let angle_of t =
  Vec.normalize_angle
    ~min_angle:(Angle.of_radians (Float.pi /. -2.))
    ~max_angle:(Angle.of_radians (Float.pi /. 2.))
...
module With_epsilon (Epsilon : Epsilon) = struct
  ...
  let are_parallel t1 t2 =
    General.imp_equals_angle (angle_of t1) (angle_of t2) ~epsilon
...
let line_of_point_slope p slope = line_of_point_angle p (Angle.atan slope)
...
let ray_of_point_slope p slope = ray_of_point_angle p (Angle.atan slope)
