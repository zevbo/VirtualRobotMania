...
let unsafe_param_of t pt =
  Vec.dist t.pt pt /. Vec.magSq t.dir_vec

let param_to_point t param =
  Vec.add t.pt (Vec.scale t.dir_vec param)
...
let is_param_on t param =
  Bool.equal ((flips_before t param) % 2 = 0) (start_on t)
...
let create_w_flip_points pt dir_vec flip_points =
  ...
  let param_of_flip flip_pt =
    ...
    | Some t -> t
    | None ->
      failwith "attempted to initialize line like with flip_pt not on line"
  in
